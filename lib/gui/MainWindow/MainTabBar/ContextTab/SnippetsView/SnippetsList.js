export class SnippetsList {
  constructor(layoutCellObject, dataProvider, contextId, codeView) {
    ...
    this._classViewMode = 'view' + this._contextId
    this._classEditMode = 'edit' + this._contextId

    this._listObject = layoutCellObject.attachList({
        type: {
            template: "<select id='tagsOfSnippet#id#' class='tagsOfSnippet " + this._classViewMode + "' multiple='multiple'>#tags#</select>",
            template_edit: "<select id='tagsOfSnippet#id#' class='tagsOfSnippet " + this._classEditMode + "' multiple='multiple'>#tags#</select>",
            height: "auto"
        }
    })
  ...
  onAfterRendered() {
    jQuery('.' + this._classViewMode).select2({
      disabled: true  // выключить компонент.
    })
  }
